...
impl Simulator {
    ...
    fn update_state(&mut self, new_request: Option<MoveRequest>) {
        if let Some(prev_req) = &self.move_request {
            let time_to_target =
                std::time::Duration::from_secs_f64((prev_req.target.0 - prev_req.origin.0).abs() as f64 / UNIT_SPEED);
            let dt = prev_req.t.elapsed();
            if dt > time_to_target {
                self.position = prev_req.target;
            } else {
                self.position = Position((prev_req.origin.0 as f64 + dt.as_secs_f64() * UNIT_SPEED) as i32);
            }
        }
        self.move_request = new_request;
    }
}
